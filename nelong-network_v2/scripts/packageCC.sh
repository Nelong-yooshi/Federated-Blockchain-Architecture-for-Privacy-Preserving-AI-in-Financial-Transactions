...
packageChaincode() {
  # set -x
  peer lifecycle chaincode package ${CHAINCODE_PATH}/${CC_NAME}_${CC_VERSION}.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ${CC_NAME}_${CC_VERSION} >&log.txt
  res=$?
  # { set +x; } 2>/dev/null
  cat log.txt
