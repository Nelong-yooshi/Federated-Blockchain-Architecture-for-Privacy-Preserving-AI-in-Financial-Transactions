...
function installChaincode() {
  ...
  if test $? -ne 0; then
    peer lifecycle chaincode install ${CHAINCODE_PATH}/${CC_NAME}_${CC_VERSION}.tar.gz >&log.txt
    res=$?
  fi
  # { set +x; } 2>/dev/null
  cat log.txt
...
function approveForMyOrg() {
  # set -x
  peer lifecycle chaincode approveformyorg -o localhost:${ORDERER_LISTEN_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  # { set +x; } 2>/dev/null
  cat log.txt
...
function commitChaincodeDefinition() {
  ...
  # set -x
  peer lifecycle chaincode commit -o localhost:${ORDERER_LISTEN_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  # { set +x; } 2>/dev/null
  cat log.txt
...
function chaincodeInvokeInit() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    peer chaincode invoke -o localhost:${ORDERER_LISTEN_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}" --isInit -c ${fcn_call} >&log.txt
    res=$?
    # { set +x; } 2>/dev/null
    let rc=$res
    COUNTER=$(expr $COUNTER + 1)
...
chaincodeInvoke() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    for ((i=1; i<=ORGS_NUM; i++)); do
        PORT=$((7051 + (i - 1) * 1000))
        PEER_CA_VAR="PEER0_ORG${i}_CA"
        PEER_CA=${!PEER_CA_VAR}  # 取得變數的值
        PEER_PARAMS+=" --peerAddresses localhost:${PORT} --tlsRootCertFiles ${PEER_CA}"
    done
    peer chaincode invoke -o localhost:${ORDERER_LISTEN_PORT} \
        -C $CHANNEL_NAME \
        -n $CC_NAME \
